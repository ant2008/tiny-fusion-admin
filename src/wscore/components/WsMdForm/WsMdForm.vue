    ...
    <ContentWrap>
      ...
      <ElTabs v-model="activeTabName">
        <ElTabPane label="明细信息" name="MD1">
          <vxe-toolbar>
            <template #buttons>
              <vxe-button status="primary" content="新增行" />
          ...
          >
            ...
            <!--            <template #action="{ row }">-->
            <!--              <ElButton @click="doUeEdit(row)" text type="primary">编辑</ElButton>-->
            <!--              <ElButton @click="doUeView(row)" text type="primary">查看</ElButton>-->
            <!--            </template>-->
          </WsTable>
        </ElTabPane>
        <!--        暂时屏蔽-->
...
import { getCurrentInstance, onMounted, PropType, ref, unref } from 'vue'
...
//获取父子表的子表数据。
const { register: tableRegister, tableObject, methods: tableMethods } = useWsTable()
...
const getDetailTableDatas = async (funcNo: string, idx: string) => {
  ...
  if (res?.dataList !== null) {
    await tableMethods.setListDatas(res.dataList)
  }
